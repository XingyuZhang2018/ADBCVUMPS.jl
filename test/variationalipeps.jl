...
@testset "non-interacting" for Ni = [2], Nj = [2]
    ...
    h = hamiltonian(model)
    as = (reshape([rand(D,D,D,D,2) for i = 1:Ni*Nj],(Ni,Nj)) for _ in 1:10)
    ...
    @test energy(h, model, SquareBCIPEPS(a); χ=30, tol=1e-10, maxiter=10)/2 ≈ -1
...
@testset "heisenberg" for Ni = [2], Nj = [2]
    ...
    bcipeps, key = init_ipeps(model; D=4, χ=30, tol=1e-10, maxiter=20)
