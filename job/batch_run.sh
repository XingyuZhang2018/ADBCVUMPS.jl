...
for i in $(seq 0.0 0.05 1.0); do cp K_gamma0.0_D${D}_chi${chi} K_gamma${i}_D${D}_chi${chi} && sed -i "7s/0.0/$i/1" K_gamma${i}_D${D}_chi${chi}; done
...
for i in $(seq 0.0 0.05 1.0); do qsub -V K_gamma${i}_D${D}_chi${chi}; done
for i in $(seq 0.0 0.05 1.0); do rm K_gamma${i}_D${D}_chi${chi}; done
