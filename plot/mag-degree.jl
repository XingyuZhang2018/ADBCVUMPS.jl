...
function magnetisation(model, folder, D, χ, tol, maxiter, miniter)
    ...
    chkp_file_up = folder*"$(model)_$(atype)/up_D$(D^2)_χ$(χ).jld2"                     
    rtup = SquareBCVUMPSRuntime(a, chkp_file_up, χ; verbose = false)   
    ALu,ARu,Cu = rtup.AL, rtup.AR, rtup.C
    chkp_file_down = folder*"$(model)_$(atype)/down_D$(D^2)_χ$(χ).jld2"                              
...
Random.seed!(100)
folder, D, χ, tol, maxiter, miniter = "./../../../../data1/xyzhang/ADBCVUMPS/K_J_Γ_Γ′/", 4, 50, 1e-10, 10, 2
